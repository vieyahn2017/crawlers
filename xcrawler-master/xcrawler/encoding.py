...
parsing.html#character-encodings-0`
...
def to_unicode(data_str, encoding):
    ...
    return data_str.decode(encoding, 'ignore')

def auto_detect(html):
    ...
    if enc:
        enc = enc.lower()
    if enc in DEFAULT_ENCODING_TRANSLATION:
        enc = DEFAULT_ENCODING_TRANSLATION[enc]
    return enc

def html_to_unicode(content_type_header, html_body_str,
                    default_encoding='utf8',
                    auto_detect_fun=auto_detect):
...
if __name__ == '__main__':
    import urllib
    c = urllib.urlopen('http://news.baidu.com').read()
    content_type_header = ''
    print html_to_unicode(content_type_header, c, default_encoding='utf8', auto_detect_fun=None)[0]
